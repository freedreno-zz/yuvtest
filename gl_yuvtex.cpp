...
static void printGLString(const char *name, GLenum s) {
    // printf("printGLString %s, %d\n", name, s);
    const char *v = (const char *) glGetString(s);
    // int error = glGetError();
    // printf("glGetError() = %d, result of glGetString = %x\n", error,
    //        (unsigned int) v);
    // if ((v < (const char*) 0) || (v > (const char*) 0x10000))
    //    printf("GL %s = %s\n", name, v);
    // else
    //    printf("GL %s = (null) 0x%08x\n", name, (unsigned int) v);
    printf("GL %s = %s\n", name, v);
}
...
#define fourcc(str) fourcc_code(str[0], str[1], str[2], str[3])
...
const int yuvTexWidth = 512;
const int yuvTexHeight = 512;
const int yuvTexUsage = GraphicBuffer::USAGE_HW_TEXTURE |
        GraphicBuffer::USAGE_SW_WRITE_RARELY;
//const int yuvTexFormat = HAL_PIXEL_FORMAT_YCrCb_422_SP; //HAL_PIXEL_FORMAT_YV12;
const int yuvTexFormat  = HAL_PIXEL_FORMAT_BGRA_8888;
...
bool setupYuvTexSurface(EGLDisplay dpy, EGLContext context) {
    ...
    if (err != 0) {
        printf("yuvTexBuffer->lock(...) failed: %d\n", err);
        return false;
    }
    ...
    if (err != 0) {
        printf("yuvTexBuffer->unlock() failed: %d\n", err);
        return false;
    }
...
int main(int argc, char** argv) {
    ...
    if(!setupYuvTexSurface(dpy, context)) {
        printf("Could not set up texture surface.\n");
        return 1;
    }
